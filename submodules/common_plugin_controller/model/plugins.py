...
class GenericPlugin(object):
    ...
    def __init__(self, info: dict, path: str, install_dependencies: bool = False) -> None:
        """
        ...
        for dependency in self.dependencies:
            if not environment_utility.check_module_availability(dependency) and (
                ...
                raise PluginImportException(self.name, self.type, f"Dependency '{dependency}' is not available!")
        if self.info.get("data_path"):
            if "./" in self.info["data_path"]:
                self.info["data_path"] = os.path.normpath(os.path.join(path, self.info["data_path"]))
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.info, dict):
            raise PluginImportException(self.name, self.type, "Plugin configuration is no valid JSON.")
        if not os.path.exists(self.path):
            raise PluginImportException(self.name, self.type, "Plugin path is not valid.")
        if self.name is None:
            raise PluginImportException("Unknown Plugin", self.type, "Plugin name not found.")
        if self.type is None:
            raise PluginImportException(self.name, "unknown type", "Plugin type not found.")
