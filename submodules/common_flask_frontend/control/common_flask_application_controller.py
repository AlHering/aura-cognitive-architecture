...
class CommonFlaskApplicationController(object):
    ...
    def __init__(self, app_config: dict) -> None:
        ...
        """
        self._logger = logging.Logger(f"[CFAController][{app_config['page_title']}]")
        ...
        self.plugin_controller = PluginController(
            ...
            plugin_folders=[cfg.PATHS.MAIN_PLUGIN_FOLDER],
    ...
    def _fix_config(self) -> None:
        ...
        for topic_index, topic in enumerate(self.config["menus"]):
            ...
            if "current_topic" not in self.config:
                self.config["current_topic"] = list(self.config["menus"].keys())[0]
    ...
    def _setup_common_routs(self) -> None:
        ...
        def register():
            if request.method == "POST":
                ...
                email_control.send_authentication_mail(form_data["register_email"].lower(), auth.uuid)
            ...
            if request.method == "GET":
                ...
                if authentication_uuid:
                    ...
                    if auth:
                        ...
                        flash("Authentication request was not successful! Please issue a new request.", "warning")
                    ...
                    form_data = request.form
                    curr_auth = database.overwrite_authentication(form_data["register_email"].lower())
                    if not curr_auth.stats:
                        email_control.send_authentication_mail(form_data["register_email"].lower(), curr_auth.uuid)
                        flash("An eMail was send to you. Please authenticate via the sent link.", "info")
                    else:
        ...
        def reset(authentication_uuid: str):
            if request.method == "GET":
                ...
                # Change authentication status, if authentication_uuid is given and authentication is not verified yet
                if authentication_uuid:
                    auth = database.get_authentication_by_uuid(authentication_uuid, "password_reset")
                    if auth and not auth.status:
                        database.verify_authentication(auth.uuid)
                        temporary_password = hashlib.md5(auth.user.email.encode("utf-8")).hexdigest()
                        database.update_user_by_id(auth.user_id,
                                                   {"password": temporary_password})
                        flash("Please log in with the temporary password and change it.", "info")
                        return render_template("reset.html", **self.config, authentication_uuid=authentication_uuid,
                    ...
                    # Get user email, overwrite authentication (if existing) and resend email
                    form_data = request.form
                    auth = database.create_new_reset_auth(form_data["reset_email"].lower(), "password_reset")
                    if auth:
                        email_control.send_authentication_mail(form_data["reset_email"].lower(), auth.uuid)
                        flash("An eMail was send to you. Please follow its instructions.", "info")
                    else:
                        flash("Request was not successful! Please issue a new request and re-check your inputs.", "warning")
                    return redirect(flask.url_for("login"), code=303)
        ...
        for blueprint_plugin in self.plugin_controller.plugins.get("blueprints", []):
            self._logger.info(f"Importing Blueprints from '{blueprint_plugin}' ...")
            if blueprint_plugin not in self.config["plugins"]:
                self.config["plugins"][blueprint_plugin] = {}
            for blueprint in self.plugin_controller.plugins["blueprints"][blueprint_plugin].get_blueprints(global_config=self.config):
                self._logger.info(f"Registering Blueprint '{blueprint}'")
                self.app.register_blueprint(blueprint)
            self._integrate_menus(self.plugin_controller.plugins["blueprints"][blueprint_plugin].get_menu())

    def _integrate_menus(self, new_menus: dict) -> None:
        """
        Internal method for integrating menus.
        :param new_menus: New menus.
        """
        for topic in new_menus:
            if topic in self.config["menus"]:
                self.config["menus"][topic].update(new_menus[topic])
            else:
                self.config["menus"][topic] = new_menus[topic]
            ...
            for key in [key for key in menu_profile[topic] if "dropdown" in menu_profile[topic][key]]:
                if menu_profile[topic][key]["href"] not in mem:
                    if menu_profile[topic][key]["href"][0] == "#":
                        ...
                        self._logger.warning(f"{menu_profile[topic][key]['href']} must start with '#'.")
                        return False
                else:
