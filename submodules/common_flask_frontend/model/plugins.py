...
class BlueprintPlugin(GenericPlugin):
    ...
    def __init__(self, info: dict, path: str) -> None:
        ...
        if "./" in self.info["blueprints"]:
            self.info["blueprints"] = os.path.normpath(os.path.join(path, self.info["blueprints"]))
        self.blueprints = environment_utility.get_module(self.info["blueprints"])
        if not hasattr(self.blueprints, "get_blueprints"):
            raise PluginImportException(self.name, self.type, "Plugin does not implement 'get_blueprints'-function correctly")
        if not hasattr(self.blueprints, "get_menu"):
            raise PluginImportException(self.name, self.type, "Plugin does not implement 'get_menu'-function correctly")
